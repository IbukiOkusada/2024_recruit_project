...
namespace
{
	const float BUTTONWIDTH = (120.0f);		//ポーズ画面ボタン幅
	const float BUTTONHEIGHT = (40.0f);		//ポーズ画面ボタン高さ
	const float BUTTONX = (640.0f);		//ポーズ画面X座標
	const float BUTTONY = (280.0f);		//ポーズ画面Y座標
	const float BUTTONSPACE = (110.0f);		//ポーズ画面縦スペース
	const float MOVECOLA = (-0.02f);		//毎フレーム変わるボタンの透明度
	const float BGSIZE = (200.0f);		//サイズ
	const float BGHEIGHT = (80.0f);		//縦幅
}
