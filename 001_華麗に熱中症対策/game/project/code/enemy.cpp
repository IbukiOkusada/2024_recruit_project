...
namespace
{
	...
	const float STATE_CNT = (300);		// 状態管理カウント
	const float DAMINTERVAL = (5);		// ダメージインターバル
	const float MAX_LIFE = (210);		// 体力
	const int STATE_LINE = (70);		// 開始状態ライン
	const float HOTINTERVAL = (10);		// 熱中症状態インターバル
	const float LEAVECNT = (120);		// 退場タイマー
	const float LEAVEMOVE = (3.0f);		// 退場時の移動量
	const float COOLDOWNCNT = (160);	// 退場前待機カウント
}
