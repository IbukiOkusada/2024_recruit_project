...
namespace
{
	const float MOVE = (2.2f);		// 移動量
	const float SHW_MOVE = (1.0f);	// シャワー中移動量
	const float PLAYER_GRAVITY = (-0.15f);		//プレイヤー重力
	const float PLAYER_JUMP = (10.0f);	//プレイヤージャンプ力
	const int DEF_SLOWTIME = (60 * 5);	// スロー可能時間
	const int SLOW_OK = (60 * 2);	// スロー可能になる時間
	const float SHOT_LENGTH = (4000.0f);	// 射程範囲
	const float SHOT_RANGE = (D3DX_PI * 0.9f);	// 射程角度
	const int SHOT_INTERVAL = (10);		// 攻撃インターバル
	const float GUN_BULMOVE = (50.0f);
	const float DEF_SLOWGAGELENGSH = (SCREEN_WIDTH * 0.4f);	// スローゲージマックスサイズ
	const float SLOWGAGE_HEIGHT = (SCREEN_HEIGHT * 0.015f);
	const char* GAGE_TEXNAME = "data\\TEXTURE\\gage000.jpg";	// ゲージファイル名
	const char* MANUAL_TEXNAME = "data\\TEXTURE\\slow_button.png";
	const float NOCHARGE_CNT = (20);		// チャージまでのカウント数
	const float BALLOON_MOVE = (25.0f);	// 風船移動量
	const float BALLOON_WEIGHT = (150.0f);	// 最大重量
	const float WIDTH = (20.0f);		// 幅
	const float HEIGHT = (80.0f);	// 高さ
	const float GAGE_SPEED = (0.001f);
}
