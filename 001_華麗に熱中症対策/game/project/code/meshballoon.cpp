...
namespace
{
	const char* TEXTUREFILE_DATA ="data\\TEXTURE\\balloon001.jpg";		//テクスチャデータ
	const float GRAVITY = (-0.1f);		// 重力
	const int SWELL_CNT = (5);	// 膨らむカウント
	const float SWELL_MOVE = (20.0f);	// 膨らむ量
	const int SHRINK_CNT = (2);		// 縮むカウント
	const float SHRINK_MOVE(-3.0f);	// 縮む量
	const int NUM_EXPLOWATER(20);	// 水しぶき数
	const float SIZEUP_WIDTH(0.05f);	// 幅
	const float MIN_SIZE(60.0f);	// 最低サイズ
}
