...
namespace
{
	const float EDITMODEL_MOVE = (5.0f);			//モデル移動量
	const float MAX_EDITSPEED = (100.0f);			//最大移動量
	const float MIN_EDITSPEED = (1.0f);				//最低移動量
	const char* EDIT_DATATXT = "data/TXT/edit.txt";	//エディットデータ保存用テキストファイル
	const float MAX_MESHLENGTH = (500.0f);			// メッシュフィールド判定最大
	const float MIN_MESHLENGTH = (20.0f);			// メッシュフィールド判定最小
	const float MAX_MESHMOVE = (100.0f);			// メッシュフィールド移動量最大
	const float MIN_MESHMOVE = (0.5f);				// メッシュフィールド移動量最小
}
