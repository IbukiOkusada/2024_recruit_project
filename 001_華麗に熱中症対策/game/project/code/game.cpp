...
namespace
{
	const int START_TIME = (60 * 4);		// 制限時間
	const int START_SCORE = (0);		    // 開始スコア
	const float MAP_SIZE = (100.0f);		// マップサイズ
	const int STARTSET_NUMENEMY = (3);  	// 開始時に配置する敵の数
	const int MORNING_TIME = (60 * 6);		// 早朝終了時刻
	const int AM_COMRUSH = (60 * 9);		// 通勤ラッシュ
	const int RELAX_TIME = (60 * 11);		// リラックスタイム
	const int NOON_TIME = (60 * 12.5);	    // 昼休み
	const int EVENT_RUSH = (60 * 15.5);		// イベント
	const int PM_RELAX_TIME = (60 * 16);	// 帰宅開始時間
	const int PM_GOHOME_RUSH = (60 * 19);	// 帰宅ラッシュ
	const int MAX_TIME = (60 * 20 + 10);	// 最大時間
	const float SPEED_UP = (30.0f);			// 速度アップ
}
