...
namespace
{
	const int GUN_DEFHEIGHT = (700);				// デフォルトから追加で画面外に生成する高さ
	const int GUN_DEFWIDTH = (GUN_DEFHEIGHT * 2);	// デフォルトから追加で画面外に生成する幅
	const int GUN_MULTIWIDTH = (2);					// 倍率
	const float GUN_MOVE = (-100.0f);				// 移動距離
	const float GUN_COLA = (0.1f);					// 透明度
}
